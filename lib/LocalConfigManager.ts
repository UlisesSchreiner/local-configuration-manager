...
class LocalConfigsManager {
  ...
  public getRestClientFile() {
    let filePath = `${this.baseConfigPath}${this.environment}.rest_client.json`;
    let rawdata = this.readConfigFile(filePath);
    return JSON.parse(rawdata);
  }
}
...
export const getRestClientFile = instance.getRestClientFile.bind(instance);
